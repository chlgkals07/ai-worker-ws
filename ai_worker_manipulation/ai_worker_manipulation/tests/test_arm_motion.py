...
TEST_POSES = {
    ...
    'forward_near':      (0.120, -0.227,  0.675, 0.000,  0.000,  0.000,  1.000),  # +6cm x
    'forward_mid':       (0.200, -0.227,  0.675, 0.000,  0.000,  0.000,  1.000),  # +14cm x
    'far_forward':       (0.250, -0.227,  0.675, 0.000,  0.000,  0.000,  1.000),  # +19cm x — stretch
    'side_near':         (0.060, -0.327,  0.675, 0.000,  0.000,  0.000,  1.000),  # -10cm y
    'side_far':          (0.060, -0.427,  0.675, 0.000,  0.000,  0.000,  1.000),  # -20cm y
    'higher':            (0.060, -0.227,  0.775, 0.000,  0.000,  0.000,  1.000),  # +10cm z
    'lower':             (0.100, -0.250,  0.625, 0.000,  0.000,  0.000,  1.000),  # -5cm z, +4cm x offset
    'diagonal':          (0.150, -0.350,  0.675, 0.000,  0.000,  0.000,  1.000),  # forward+side
    'high_forward':      (0.150, -0.250,  0.775, 0.000,  0.000,  0.000,  1.000),  # forward+up
    'high_side':         (0.060, -0.350,  0.775, 0.000,  0.000,  0.000,  1.000),  # side+up
    'lower_side':        (0.100, -0.350,  0.625, 0.000,  0.000,  0.000,  1.000),  # side+down

    # ── Z-axis rotation (wrist yaw) ────────────────────────────────────────────
    'rot_z_neg90':       (0.060, -0.227,  0.675, 0.000,  0.000, -0.707,  0.707),  # -90°
    'rot_z_neg45':       (0.060, -0.227,  0.675, 0.000,  0.000, -0.383,  0.924),  # -45°
    'rot_z_45':          (0.060, -0.227,  0.675, 0.000,  0.000,  0.383,  0.924),  # +45°
    'rot_z_90':          (0.060, -0.227,  0.675, 0.000,  0.000,  0.707,  0.707),  # +90°

    # ── Y-axis rotation (EE pitch — tilt forward/back) ─────────────────────────
    'rot_y_30':          (0.060, -0.227,  0.675, 0.000,  0.259,  0.000,  0.966),  # 30°
    'rot_y_45':          (0.060, -0.227,  0.675, 0.000,  0.383,  0.000,  0.924),  # 45°
    'rot_y_90':          (0.060, -0.227,  0.675, 0.000,  0.707,  0.000,  0.707),  # 90° — stretch goal

    # ── X-axis rotation (EE roll) ──────────────────────────────────────────────
    'rot_x_30':          (0.060, -0.227,  0.675, 0.259,  0.000,  0.000,  0.966),  # +30°
    'rot_x_neg30':       (0.060, -0.227,  0.675,-0.259,  0.000,  0.000,  0.966),  # -30°

    # ── Combined position + orientation (practical grasping scenarios) ─────────
    'fwd_rot_z45':       (0.150, -0.250,  0.675, 0.000,  0.000,  0.383,  0.924),  # reach + yaw
    'fwd_rot_z_neg45':   (0.150, -0.250,  0.675, 0.000,  0.000, -0.383,  0.924),  # reach + -yaw
    'fwd_rot_z90':       (0.150, -0.250,  0.675, 0.000,  0.000,  0.707,  0.707),  # reach + 90° yaw
    'fwd_rot_z_neg90':   (0.150, -0.250,  0.675, 0.000,  0.000, -0.707,  0.707),  # reach + -90° yaw
    'fwd_rot_y30':       (0.150, -0.250,  0.675, 0.000,  0.259,  0.000,  0.966),  # reach + pitch
    'fwd_rot_x30':       (0.150, -0.250,  0.675, 0.259,  0.000,  0.000,  0.966),  # reach + roll
    'side_rot_z90':      (0.060, -0.350,  0.675, 0.000,  0.000,  0.707,  0.707),  # side + 90° yaw
    'lower_rot_z45':     (0.100, -0.250,  0.625, 0.000,  0.000,  0.383,  0.924),  # lower + yaw
    'diagonal_rot_z45':  (0.150, -0.350,  0.675, 0.000,  0.000,  0.383,  0.924),  # diagonal + yaw
}

# Left arm poses — y coordinates are positive (mirrored from TEST_POSES).
# READY_POSE_L = (0.060, +0.227, 0.675); side direction is +y for arm_l.
TEST_POSES_L = {
...
# Cartesian pose arm_r is driven to before each right-arm test.
_READY_POSE_XYZQ   = (0.060, -0.227, 0.675, 0.0, 0.0, 0.0, 1.0)
# Cartesian pose arm_l is driven to before each left-arm test (y mirrored).
...
# Once MoveIt is running, use RViz → MotionPlanning → Joints tab to jog arm_r to a clear
# "ready" pose, read the joint values, and replace HOME_R here.
HOME_R = [0.0, -0.5, 0.0,  0.5, 0.0, 0.0, 0.0]   # joint2 away from boundary
HOME_L = [0.0,  0.5, 0.0, -0.5, 0.0, 0.0, 0.0]   # mirror for left
...
class MotionTest:
    ...
    def _home(self) -> bool:
        """Drive arm_r to READY_POSE between tests via move_to_pose.

        move_to_joints is avoided: the follower-mode arm_r_controller rejects
        joint-space goals from MoveIt. Cartesian homing works and ensures joint2
        is pulled away from its zero upper-limit before each IK solve.
        """
    ...
    def _home_l(self) -> bool:
        """Drive arm_l to its READY_POSE_L between tests via move_to_pose."""
    ...
    def section_current_state(self) -> None:
        """Print FK pose and joint positions for both arms — use to calibrate TEST_POSES."""
    ...
    def section_pose_sweep(self) -> None:
        """Test each pose in TEST_POSES with the default planner (RRTConnect)."""
    ...
    def section_pose_sweep_left(self) -> None:
        """Test each pose in TEST_POSES_L with the default planner (RRTConnect)."""
