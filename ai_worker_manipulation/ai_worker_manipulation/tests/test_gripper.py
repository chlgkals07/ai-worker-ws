"""
Gripper test — open / close on right, left, and both simultaneously.
Includes a grasp assessment section (hardware only — requires object_lut.json).

NOTE: if the gripper does nothing and logs show no errors, the joint_trajectory
topic is silently remapped by the follower_ai launch. See TODO in gripper_command.py.

Usage:
    ros2 run ai_worker_manipulation test_gripper
"""
...
class GripperTest:
    ...
    # ------------------------------------------------------------------
    # Helpers — same pattern as test_moveit_client.py
    # ------------------------------------------------------------------
    ...
    def _wait(self) -> None:
        """Wait for command dispatch then physical motion — uses GripperInterface methods."""
        self._gripper.wait_until_executed()
        self._gripper.wait_motion()

    def _pos(self, side: str) -> str:
        """Format current position for logging."""
    ...
    def section_right(self) -> None:
        ...
        # is_open / is_closed before we touch it
    ...
    def section_both(self) -> None:
        ...
        # GripperCommand.send('both', ...) publishes to both controllers at once.
        # GripperInterface.open/close('both') routes through to that.
    ...
    def _run_assess(self, side: str) -> dict | None:
        """Run assess() and return result dict, or None on error."""
        try:
    ...
    def section_grasp_assessment(self) -> None:
        ...
        # ── Part B: interactive object test (hardware only) ───────────
        self._log.info('')
