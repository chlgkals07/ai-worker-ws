...
pick_server.py
--------------
ROS 2 Action Server for /pick.
Executes pick phase only (capture pose → GPD → grasp → inspection pose).
Returns arm used, for caller to pass to /place.
"""
...
from ai_worker_manipulation_msgs.action import Pick
...
class PickServer(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info('[PickServer] ready')

    def _goal_cb(self, goal_request):
        if self._busy.locked():
            self.get_logger().warn('[PickServer] busy — rejecting goal')
    ...
    def _execute_cb(self, goal_handle):
        with self._busy:
            ...
            pick_result = self._orchestrator.pick(
                object_class=goal.object_class,
                feedback_cb=_feedback,
            )

            result                = Pick.Result()
            result.success        = pick_result.success
            result.failure_reason = '' if pick_result.success else pick_result.result.value
            result.arm            = pick_result.arm.value if pick_result.arm else ''
