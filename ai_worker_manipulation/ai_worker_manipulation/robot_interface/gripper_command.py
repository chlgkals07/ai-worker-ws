...
class GripperCommand:
    ...
    # Gripper joints live inside the arm controllers.
    ...
    # Trajectory execution time. Increase if motion is unstable.
    MOTION_TIME  = 1.0
    POSITION_MIN = 0.0
    POSITION_MAX = 1.0
    ...
    def __init__(self, node, callback_group=None):
        ...
        self._pubs = {
            'left': self._node.create_publisher(
                JointTrajectory, self.CONTROLLER_TOPICS['left'], 10,
            ),
            'right': self._node.create_publisher(
                JointTrajectory, self.CONTROLLER_TOPICS['right'], 10,
            ),
        }
    ...
    def _validate_position(self, func_name: str, position: float) -> bool:
        """Logs a warning if out of normalized 0.0–1.0 range."""
    ...
    def _send_single(self, side, position) -> bool:
        """
        ...
        point.time_from_start = Duration(seconds=self.MOTION_TIME).to_msg()
        ...
        msg.joint_names = self.ARM_JOINTS[side] + [self.GRIPPER_JOINTS[side]]
