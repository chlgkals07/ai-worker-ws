...
class GripperInterface:
    ...
    def __init__(self, node, ignore_new_calls_while_executing: bool = True):
        self._node    = node
    ...
    def toggle(self, side):
        if self.is_open(side):
            self.close(side, skip_if_noop=False)
        else:
            self.open(side, skip_if_noop=False)

    def control(self, side, position: float):
        side = side.lower()
        with self._lock:
            if self._ignore_new_calls_while_executing and self._is_executing:
                return
            self._is_motion_requested = True
            self._is_executing        = True
        self._start_execution(self._command.send, side, position)

    def open(self, side, skip_if_noop=False):
        side = side.lower()
        if skip_if_noop and self.is_open(side):
            return
        self.control(side, self.OPEN)

    def close(self, side, skip_if_noop=False):
        side = side.lower()
        if skip_if_noop and self.is_closed(side):
            return
        self.control(side, self.CLOSE)
    ...
    def wait_until_executed(self):
        if not self._is_motion_requested and not self._is_executing:
            return False
        while self._is_motion_requested or self._is_executing:
            time.sleep(0.001)
        return True
    ...
    def is_open(self, side):
        side     = side.lower()
        position = self._command.get_position(side)
        if position is None:
            return True
